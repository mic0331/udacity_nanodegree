...
date: "Last edited, Thursday, May 15, 2015"
output:
  html_document:
    toc: true
    highlight: default
...
airports (WRD, ASE) have very low average delay despit the number of fligh 
...
```{r}
# We can also produce a heatmap with the average over the years and months.
# split the data per seasons
...
#ne note: 2012 is a good year to which to convert all of the dates; since it is a leap year, any February 29ths in your data set will be handled smoothly.

rita.flight_delay_15  <- mutate(rita.flight_delay_15,
       date = as.Date(paste(Year, Month, DayofMonth, sep='-')))
rita.flight_delay_15  <- mutate(rita.flight_delay_15,
        season = getSeason(date))

A <- rita.flight_delay_15
B <- group_by (A, daycol, weekrow, season)
C <- summarise(B, flight_delay = mean(proportion)  )
ggplot(C, aes(daycol, weekrow, fill = logit(flight_delay))) +
geom_tile() +
facet_wrap(~ season) +
theme_bw()
```
...
average delay of 93 minutes (we consider this airport as an outlier and therefore 
it has been removed from the drawing).
...
dottedYears <- seq(as.Date("1991/1/1"), 
                   as.Date("2008/1/1"), 
                   by = "5 year")
# The y-axis depicts the ratio between min and max delay. In that case the units on the y-axis are numbers not hours.
...
g.bottom <- ggplot(rita.flight_delay_plot_two) +
  ...
  labs(x = "Year", y = "Min / Max Delay (number)") +
  ...
  annotate("text", 
           x=as.Date("1987/5/1"), 
           y=13, 
           label="show the min/max delay for every day and            ", 
           size=3, color="gray30", hjust = 0) + 
  annotate("text", 
           x=as.Date("1987/5/1"), 
           y=12, 
...
This plot is showing the increase of arrival delays from multiple angles.  The 
y-axis depicts the ratio between min and max delay. This information is shown as 
bars for every days and with a red/brun line for each year.  
We see that the minimum delay is not really increasing or decreasing from year 
to year but the maximum delay is on the other hand, increasing regularly.  We 
certainly see the highest increase when we left the 19th.
